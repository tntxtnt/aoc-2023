...
std::vector<std::vector<int64_t>> cart_product(const std::vector<std::vector<int64_t>>& v) {
...
std::vector<int64_t> getDivisors(int64_t n) {
    ...
    for (auto& g : cart_product(powers)) res.push_back(std::accumulate(begin(g), end(g), 1LL, std::multiplies{}));
    ranges::sort(res);
    return res;
}
...
int64_t part2(Input input) {
    ...
    for (auto [vx, vy, vz] : views::cartesian_product(velX, velY, velZ)) {
        std::vector<int64_t> times(input.size());
        const auto& a = input[0];
        if (!ranges::all_of(views::iota(size_t{1}, input.size()), [&](size_t i) {
                const auto& b = input[i];
                const auto p = integerIntersection(a.pos.x, a.pos.y, vx - a.vel.x, vy - a.vel.y, //
                                                   b.pos.x, b.pos.y, vx - b.vel.x, vy - b.vel.y);
                if (!p) return false;
                const auto t1 = static_cast<int64_t>(std::round(p->first));
                const auto t2 = static_cast<int64_t>(std::round(p->second));
                if (t1 >= 0 || t2 >= 0) return false;
                if (times[0] == 0) times[0] = t1;
                if (t1 != times[0]) return false;
                times[i] = t2;
                return true;
            }))
            continue;
        if (![](auto v) {
            ...
            continue;
        const auto x = a.pos.x + (vx - a.vel.x) * times[0];
        const auto y = a.pos.y + (vy - a.vel.y) * times[0];
        const auto z = a.pos.z + (vz - a.vel.z) * times[0];
